...
//--- Flags for create CMS ---
#define KCS_SIMPLE_CMS				0x00001001
#define KCS_ARCHIVAL_CMS			0x00001002
#define KCS_GET_HASH_FROM_XML		0x00001003
#define KCS_GET_XAdES_FROM_HASH		0x00001004
...
#define KC_VRF_OCSP_RESPONSE		0x00400000
#define KC_HASH_SHA1				0x00800000
...
#define KCR_OCSP_CHECKCERTFROMRESP				KCR_BASE + 0x0000005c
#define KCR_CRLEXPIRED							KCR_BASE + 0x0000005d
#define KCR_ADDENTRYTOX509NAME					KCR_BASE + 0x0000005e
#define KCR_ADDSUBJKEYID						KCR_BASE + 0x0000005f
#define KCR_CREATEX509REQ						KCR_BASE + 0x00000060
#define KCR_PUBKEY_PARSE_ERROR					KCR_BASE + 0x00000061
...
typedef struct stKCFunctions {
	...
	unsigned long (*SignDataArchive)(char *alias, int flags, char *inData, int inDataLength, unsigned char *inSign, int inSignLen, char *validPath, char* container_for_CRL, unsigned char *outSign, int *outSignLength);
	...
	unsigned long (*KC_InsertTStoCMS)(int flags, char *inCMS, int inCMSLength, char *outSign, int *outSignLen, char *outTS, int *outTSLen);

	unsigned long(*KC_GetSignInfosFromDoc)(char *inCert, int inCertLen, char *inData, int inDataLength, char *outSignInfos, int *outSignInfosLen, char *outHash, int *outHashLen);
	unsigned long(*KC_CreateCMSfromDraftSign)(int flags, char *inCert, int inCertLen, char *inData, int inDataLength, char *inDraftSign, int inDraftSignLen, char *inSignInfo, int inSignInfoLen, char *OCSP_resp, int OCSP_respLen,  char *outSign, int *outSignLen, char *outTS, int *outTSLen);
	unsigned long(*KC_GetTSfromDraftSign)(char *inCert, int inCertLength, char *inDraftSign, int inDraftSignLength, char *outTS, int *outTSLen);
	unsigned long(*KC_CreateXAdESfromCloudStorage)(int flags, char *inCert, int inCertLen, char *inData, int inDataLength, char *inDraftSign, int inDraftSignLength, char *outData, int *outDataLen);

	unsigned long(*KC_GenerateX509ReqFL)(int digestAlg, char *pubkey_string, char *commonName, char *surname, char *givenName, char *serialNumber, char *countryName, char *localityName, char *stateOrProvinceName, char *emailAddress, char *outX509Req, int *outX509ReqLen, char *outDataHash, int *outDataHashLength);
	unsigned long(*KC_AddSignatureToX509Req)(int digestAlg, char *x509_req_string, char *signature_string, char *outX509Req, int *outX509ReqLen);
